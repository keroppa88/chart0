...
//d.Code (銘柄コード)
//d.NP (当期純利益)Profit
//d.EPS (一株あたり当期純利益)EarningsPerShare
//d.BPS (一株あたり純資産)BookValuePerShare
//d.FDivAnn (一株あたり配当予想_合計)ForecastDividendPerShareAnnual
...
async function fetchAndSave(code) {
  ...
  try {
    ...
    if (allData.length > 0) {
      // 最後の8行のみ取得(直近2年分に相当）
      // const last8Data = allData.slice(-8);
      // 全期間とる
      // const last8Data = allData; 
      
      const last8Data = allData;      
      const csvContent = convertToCsv(last8Data);
      fs.writeFileSync(path.join(DATA_DIR, `${code}.csv`), csvContent);
      console.log(`✅成功: ${code}_finance.csv (全${allData.length}件中、最新${last8Data.length}件を保存)`);
    } else {
      console.log(`⚠️ データなし: ${code}`);
    }
  } catch (e) {
 ...
 */
async function main() {
  console.log("財務データ取得を開始します...");
  console.log("(最新●件のみ保存)");
  console.log("---");
